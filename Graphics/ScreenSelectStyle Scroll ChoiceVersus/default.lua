...
t[#t+1] = Def.ActorFrame {
		LoadActor("Info") .. {
		Condition=GAMESTATE:GetNumPlayersEnabled()==2;
	  InitCommand=cmd(diffusealpha,0;x,344;y,-150;zoom,2);
		OnCommand=cmd(sleep,0.7;zoomy,0;diffusealpha,0;linear,0.3;diffusealpha,1;zoomy,2);
		GainFocusCommand=cmd(stoptweening;sleep,0.1;linear,0.2;zoomy,2);
		LoseFocusCommand=cmd(stoptweening;linear,0.2;zoomy,0);
		OffCommand=cmd(sleep,0.3;linear,0.1;zoomy,0;diffusealpha,0;);
	};
	    LoadActor("Info") .. {
		Condition=GAMESTATE:GetNumPlayersEnabled()==1;
	  InitCommand=cmd(diffusealpha,0;x,344;y,-150;zoom,2);
		OnCommand=cmd(sleep,0.1;zoomy,0;diffusealpha,0;linear,0.3;diffusealpha,0;zoomy,2);
		GainFocusCommand=cmd(stoptweening;sleep,0.1;linear,0.2;zoomy,1.98;diffusealpha,1);
		LoseFocusCommand=cmd(stoptweening;linear,0.2;zoomy,0);
		OffCommand=cmd(sleep,0.3;linear,0.1;zoomy,0;diffusealpha,0;);
	};
};
t[#t+1] = Def.ActorFrame {
	LoadActor("Title") .. {
		...
		OffCommand=cmd(stoptweening;sleep,0.3;linear,0.1;zoom,0;diffusealpha,0;);
	};
	LoadActor("Title") .. {
	    Condition=GAMESTATE:GetNumPlayersEnabled()==1;
