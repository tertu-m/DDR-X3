return Def.ActorFrame{
	Def.ActorFrame{
		InitCommand=cmd(player,PLAYER_1;x,-250;y,100);
		LoadActor("1P_OK")..{
		...
		--	ExitUnselectedP1Command=cmd(stoptweening;diffusealpha,0;zoomy,0);
			ExitSelectedP1Command=function(self)
				...
				(cmd(stoptweening;bob;effectmagnitude,0,2,0;effectperiod,1;bounceend,0.2;diffusealpha,1;zoom,1.4))(self)
			end;
		...
		LoadActor("1P_WAIT")..{
			...
			OffCommand=cmd(zoom,1.4);
		};
	};
	Def.ActorFrame{
		InitCommand=cmd(player,PLAYER_2;x,250;y,100);
		LoadActor("2P_OK")..{
			InitCommand=cmd(diffusealpha,0;zoomy,0);
